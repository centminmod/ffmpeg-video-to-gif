...
vid2gif_pro() {
    ...
    # Default MP4 codec if none specified alongside --to-mp4-* (shouldn't happen with new flags)
    ...
    if [[ "$output_format" == "gif" ]]; then
        ...
        # (Code for GIF conversion remains the same as previous version)
        local palette_file=""
        ...
        if ! "${palettegen_cmd_array[@]}"; then
           ...
           exit_code=1
            ...
            local gifgen_cmd_array=("ffmpeg" "-y" "-v" "quiet" "-i" "$src" "-i" "$palette_file" "-filter_complex" "[0:v]${filters}[s]; [s][1:v]paletteuse=${paletteuse_options}" "$target")
        ...
        echo "Converting to MP4 ($video_codec)... This might take a while for AV1."
        ...
        # Codec options - AV1 often uses -cpu-used instead of -preset
        mp4_cmd_array+=( "-c:v" "$video_codec" )
        if [[ "$video_codec" == "libaom-av1" ]]; then
            # Example: add -cpu-used 4 (range 0-8, higher is faster/lower quality)
            # For simplicity, we'll omit it unless you want to add another flag like --av1-cpu-used
...
# 5. Ensure ffmpeg (with libaom enabled for AV1) and gifsicle are installed.
